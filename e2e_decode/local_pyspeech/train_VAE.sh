...
if ${only_extract_egs}; then
  echo "$0: Only extracting egs, skipping model training...."
  exit 0;
fi
...
if ${shared_decoder_var} ; then
   add_vae_opts="$add_vae_opts --shared_decoder_var"
fi
