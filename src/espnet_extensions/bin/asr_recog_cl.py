...
def get_parser():
    ...
    # MODEL OUTPUT FUSION OPTIONS
    parser.add_argument(
        "--model_list",
        type=str,
        default=None,
        help="List of ASR models for multi-stream CL",
    )
    parser.add_argument(
        "--pm_list",
        type=str,
        default=None,
        help="List of confidence models for multi-stream CL",
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=50,
        help="Attention temperature for multi-stream CL",
    )
    parser.add_argument(
        "--pm_type",
        type=str,
        default="vae",
        choices=["vae", "vae_old", "ae", "weighted", "mmeasure", "mmeasure_addlm", "joint_vae", "true_joint_vae", "enc_vae", "enc_vae_ff", "vae_ff", "average"],
        help="Type of confidence models for multi-stream CL",
    )
    parser.add_argument(
        "--weight_combination",
        type=str,
        default="standard",
        choices=["standard", "alternative", "per_frame", "per_frame_logits", "per_frame_posteriors"],
        help="Standard computes mean of the score and then use softmax to obtain the weight, alternative first computes softmax over models and then computes sum over time. Per_frame does combination on each frame, using either VAE score, logits or posteriors.",
    )
    parser.add_argument(
        "--average_ctc",
        action='store_true',
        help="Combine ctcs only by averaging scores.",
    )
    parser.add_argument(
        "--average_attn",
        action='store_true',
        help="Combine attentions only by averaging scores.",
    )
    parser.add_argument(
        "--ignore_attn",
        action='store_true',
        help="Hack to do beam search with LM for CTC (how model should behave when ctc-weight: 1.0).",
    )
    parser.add_argument(
        "--score_type",
        type=str,
        default="prod",
        choices=["sum", "prod"],
        help="Sum or product scoring",
    )
    parser.add_argument(
        "--pm_weights",
        type=str,
        default=None,
        help="PM weights, this will only be used if pm_type is 'weighted'",
    )
    parser.add_argument(
        "--pm_save_path",
        type=str,
        default=None,
        help="File to save the pm weights",
    )
    parser.add_argument(
        "--posterior_save_path",
        type=str,
        default=None,
        help="File to save posteriors",
    )
    parser.add_argument(
        "--rule",
        type=str,
        default='product',
        choices=["sum", "product"],
        help="Type of ensemble rule, supported rules are 'product' OR 'sum'",
    )
    parser.add_argument(
        "--general_p",
        type=float,
        default=None,
        help="If a general_p value is provided, will computed generalized mean combination of classifiers with parameter general_p",
    )
    parser.add_argument(
        "--update_temp",
        action='store_true',
        help="Set to automatically update the temperature",
    )
    parser.add_argument(
        "--oracle_wts",
        type=str,
        default=None,
        help="If oracle weights are given, will ignore all performance monitoring",
    )
    parser.add_argument(
        "--pm_list_enc",
        type=str,
        default=None,
        help="List of confidence models for multi-stream CL for encoder",
    )
    parser.add_argument(
        "--enc_feat_transform",
        type=str,
        default=None,
        help="List of encoder feature CMVN/PCA transformation files (generated by kaldi)",
    )
    parser.add_argument(
        "--enc_feat_transform_type",
        type=str,
        choices=["cmvn", "pca"],
        default="cmvn",
        help="Set to specify use of pca matrices instead of cmvn normalization",
    )
    parser.add_argument(
        "--chop",
        type=float,
        default=None,
        help="Time in seconds of utterance to use for PM computation ( option not used normally)",
    )
    parser.add_argument(
        "--auto_vad",
        action='store_true',
        help="Set to automatically exclude silences from vae loss computation",
    )
    parser.add_argument(
        "--auto_vad_threshold",
        type=float,
        default=0.4,
        help="Threshold to be set for automatic VAD to exclude silence",
    )
    parser.add_argument(
        "--vae_output_distribution",
        type=str,
        choices=["gauss","laplace"],
        default="gauss",
        help="Output distribution of the VAE model",
    )
        ...
        if args.backend == "chainer":
            ...
            if args.num_encs == 1:
                ...
                if args.api == "v2":
                    ...
                    if args.pm_type == "joint_vae" or args.pm_type == "enc_vae" or args.pm_type == "true_joint_vae" or args.pm_type == "enc_vae_ff":
                        from src.espnet_extensions.asr.asr import recog_joint_cl


                        recog_joint_cl(args)
                    else:
                        from src.espnet_extensions.asr.asr import recog_cl
